...
# --- 顏色空間轉換輔助函數 (LAB) ---
# [Code for rgb_to_xyz, xyz_to_lab, lab_to_xyz, xyz_to_rgb, rgb_to_lab, lab_to_rgb]
def rgb_to_xyz(rgb):
    mask = rgb > 0.04045
...
class RGBtoRYGCBM:
    ...
    def convert(self, image):
        ...
        R = torch.maximum(R*2 - Y/2 - M/2, torch.zeros_like(R))
        G = torch.maximum(G*2 - Y/2 - C/2, torch.zeros_like(G))
        B = torch.maximum(B*2 - C/2 - M/2, torch.zeros_like(B))
...
class RYGCBMtoRGB:
    ...
    def convert(self, image, blend_mode, normalize):
        ...
        if blend_mode == "average":
            ...
            result[..., 0] = torch.maximum(R, torch.maximum(Y/2, M/2))
            result[..., 1] = torch.maximum(G, torch.maximum(Y/2, C/2))
            result[..., 2] = torch.maximum(B, torch.maximum(C/2, M/2))
        else:
            result[..., 0] = torch.minimum(R, torch.minimum(Y/2, M/2))
            result[..., 1] = torch.minimum(G, torch.minimum(Y/2, C/2))
            result[..., 2] = torch.minimum(B, torch.minimum(C/2, M/2))
...
class ExtractImageChannel:
    ...
    def extract(self, image, channel, output_mode):
        ...
        if output_mode == "single_channel":
            ...
            result = torch.zeros((batch, height, width, 3), device=image.device, dtype=image.dtype)
            result[..., 0] = selected_channel
            result[..., 1] = selected_channel
            result[..., 2] = selected_channel
            return (result,)
...
# --- 核心 ImageStitcher 類別 (已修正 Matches Visualization 尺寸邏輯) ---
...
class ImageStitcher:
    ...
    def draw_matches(self, imageA, imageB, kpsA, kpsB, matches, status):
        ...
        # 匹配圖是兩張圖橫向拼接
        # 由於兩張輸入圖在特徵檢測階段已經統一縮放尺寸，hA == hB, wA == wB
    ...
    def stitch(self, image1, reference, feature_detection_size_mode, upscale_method, ratio, reproj_thresh, show_matches, output_size_mode):
        ...
        if M is None:
            ...
            # 即使 fallback，matches_vis 也要與 stitched_image 尺寸匹配
            if output_size_mode == "image1":
                matches_vis_tensor = torch.zeros_like(image1)
            else:
                matches_vis_tensor = torch.zeros_like(reference)
